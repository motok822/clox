...
static bool call(ObjClosure *closure, int argCount)
{
    ...
    {
        runtimeError("Runtime error: Expected %d arguments but got %d.", closure->function->arity, argCount);
        return false;
    }

    if (vm.frameCount == FRAMES_MAX)
    {
        runtimeError("Runtime error: Stack overflow.");
        return false;
    }
...
static bool callValue(Value callee, int argCount)
{
    if (IS_OBJ(callee))
    {
        ...
        case OBJ_CLASS:
        {
            ...
            vm.stackTop[-argCount - 1] = OBJ_VAL((Obj *)newInstance(kclass));
            Value initializer;
    ...
    runtimeError("Runtime error: Can only call functions and classes.");
    return false;
}
...
static bool bindMethod(ObjClass *klass, ObjString *name)
{
    ...
    {
        runtimeError("Runtime error: Undefined property '%s'.", name->chars);
        return false;
    }
...
static bool invoke(ObjString *name, int argCount)
{
    ...
    {
        runtimeError("Runtime error: Only instances have methods.");
        return false;
    }
...
#define READ_STRING() AS_STRING(READ_CONSTANT())
    for (;;)
    {
        ...
        case OP_NEGATE:
            if (!IS_NUMBER(peek(0)))
            {
                printf("Runtime error: Operand must be a number.\n");
                return INTERPRET_RUNTIME_ERROR;
            }
        ...
        case OP_GET_PROPERTY:
        {
            ...
            {
                runtimeError("Runtime error: Only instances have properties.");
                return INTERPRET_RUNTIME_ERROR;
            }
        ...
        case OP_SET_PROPERTY:
        {
            ...
            {
                runtimeError("Runtime error: Only instances have fields.");
                return INTERPRET_RUNTIME_ERROR;
            }
        ...
        case OP_INHERIT:
        {
            ...
            {
                runtimeError("Runtime error: Superclass must be a class.");
                return INTERPRET_RUNTIME_ERROR;
            }
...
{
    ObjFunction *function = compile(source, vm.chunk);
    if (function == NULL)
